		...
		if (isset($block['blockName'])) {
			...
			if (!empty($block['attrs']['className'])) {
				if ($blockname == 'core/button') {
					...
					if (str_contains($block['attrs']['className'], 'is-style-gs-creativeoverlay') !== false) {
						...
						$block_style .= '.is-style-gs-brdnpaddradius > ul > li{border-radius:8px; padding:15px;box-sizing: border-box;border: 1px solid var(--wp--preset--color--lightborder);margin-bottom:1.25em}.editor-styles-wrapper .is-style-gs-brdnpaddradius figure.wp-block-post-featured-image, .is-style-gs-brdnpaddradius figure.wp-block-post-featured-image{margin:-15px -15px 12px -15px !important;border-radius:8px 8px 0 0}';
					}
			...
			if (!empty($block['attrs'])) {
				if ($blockname == 'core/cover' || $blockname == 'core/column' || $blockname == 'core/columns' || $blockname == 'core/group'|| $blockname == 'core/template-part' || $blockname == 'core/image' || $blockname == 'core/button' || $blockname == 'core/site-title' || $blockname == 'core/site-logo') {
                    ...
                    if ($block['blockName'] == 'core/group' && !empty($block['attrs']['gsStickyHeader'])) {
                        wp_enqueue_style('greenshift-stickyheader');
                        wp_enqueue_script('greenshift-stickyheader');
                    }
                    if ($block['blockName'] == 'core/template-part' && !empty($block['attrs']['gsHeaderAlpha'])) {
                        $block_style .= '.site-header{left: 0;right: 0;position:fixed; top:0;}.has-modal-open .site-header{z-index: 100001;}.site-header:not(.gs-sticky-enable) > div{background: transparent !important}.site-header:not(.gs-sticky-enable){z-index:999;}#wpadminbar ~ .wp-site-blocks .site-header:not(.gs-sticky-enable){top:32px;}';
                    }
				}
			}
		}
